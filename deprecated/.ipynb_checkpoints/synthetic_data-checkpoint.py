...
def generate_lorenz_data(N_trials, N_inits, N_cells, N_steps, N_stepsinbin = 1,
                         ...
                         trainp= 0.8, seed=100,
                         do_oasis = 'known', save=True, save_dir='./'):
    ...
    if do_oasis is not None:
        if importlib.find_loader('oasis'):
            if do_oasis == 'known':
                train_S, train_C = deconvolve_calcium_known(data_dict['train_fluor'], g=np.exp(-dt_cal/tau_c), s_min=0.5)
                valid_S, valid_C = deconvolve_calcium_known(data_dict['valid_fluor'], g=np.exp(-dt_cal/tau_c), s_min=0.5)
                obs_tau_init = tau_c
                obs_var_init = sigma**2
                obs_gain_init = 1.0
                obs_bias_init = 0
            elif do_oasis == 'unknown':
                train_S, train_C = deconvolve_calcium_unknown(data_dict['train_fluor'], fps=1./dt_cal, tau_init=tau_c, snr_thresh=3)
                valid_S, valid_C = deconvolve_calcium_unknown(data_dict['valid_fluor'], fps=1./dt_cal, tau_init=tau_c, snr_thresh=3)
            
            data_dict['train_ospikes'] = train_S
            data_dict['train_ocalcium'] = train_C
            data_dict['valid_ospikes'] = valid_S
            data_dict['valid_ocalcium'] = valid_C
